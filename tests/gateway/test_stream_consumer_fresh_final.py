...
# ``_is_preview_stale`` measures age as ``time.monotonic() - _message_created_ts``,
# and on Linux ``time.monotonic()`` is seconds since boot. Setting the timestamp
# to 0.0 therefore makes the preview exactly as old as the machine's uptime —
# fine on a long-lived laptop, but on a freshly booted CI runner with under a
# minute of uptime the preview still looks fresh, the fresh-final path never
# fires, and the send count comes back 1 instead of 2. Anchor the age to the
# same clock the implementation reads so it is stale on any machine.
_ANCIENT = 3600.0


def _make_preview_stale(consumer) -> None:
    """Backdate the preview far enough that fresh-final always triggers."""
    consumer._message_created_ts = time.monotonic() - _ANCIENT
